...
struct GuiOutputData
{
	...
	std::atomic<int> position { 0 }; // Current playback position for visual cursor
	
	// Grain display data (positions and volumes for active grains)
	static constexpr size_t max_grains = 16;
	std::array<std::atomic<bool>, max_grains> grain_active;
	std::array<std::atomic<float>, max_grains> grain_position;
	std::array<std::atomic<float>, max_grains> grain_volume;
